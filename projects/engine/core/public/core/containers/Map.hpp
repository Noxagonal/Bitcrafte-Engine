...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template<typename KeyType, typename ValueType, bool IsConst>
...
public:
	...
	using ContainedPairType					= Pair<KeyType, ValueType>;
	using value_type						= ContainedPairType;	// for stl compatibility.

private:

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	template<typename OtherKeyType, typename OtherValueType, bool IsOtherConst>
	friend class MapIteratorBase;

	friend class MapIteratorBase<KeyType, ValueType, true>;
	friend class MapIteratorBase<KeyType, ValueType, false>;

	using Container				= Map<KeyType, ValueType>;
	using Node					= container_bases::MapNode<KeyType, ValueType>;

public:

	constexpr MapIteratorBase() noexcept = default;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	template<bool IsOtherConst>
	constexpr MapIteratorBase(
		const MapIteratorBase<KeyType, ValueType, IsOtherConst>				&	other
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	constexpr const ContainedPairType																&	operator*() const
	{
		return *this->Get();
	}

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	constexpr ContainedPairType																		&	operator*() requires( IsConst == false )
	{
		return *this->Get();
	}
	...
	constexpr bool																						operator==(
		MapIteratorBase<KeyType, ValueType, IsOtherConst>							other
		) const noexcept
	{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	constexpr MapIteratorBase													&	operator++()
	{
		BAssert( this->container, U"Tried using iterator that points to nothing" );
		BAssert( !this->container->IsEmpty(), U"Tried iterating over an empty container" );
		BAssert(
			this->node,
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	constexpr const ContainedPairType																*	Get() const
	{
		BAssert( this->container, U"Tried using iterator that points to nothing" );
		BAssert( !this->container->IsEmpty(), U"Tried iterating over an empty container" );
		BAssert(
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	constexpr ContainedPairType																		*	Get() requires( IsConst == false )
	{
		BAssert( this->container, U"Tried using iterator that points to nothing" );
		BAssert( !this->container->IsEmpty(), U"Tried iterating over an empty container" );
		BAssert(
...
private:
	...
	{
		BAssert( from_node, U"Empty node" );
...
public:
	...
	using ThisContainerType					= Map<OtherKeyType, OtherValueType>;
	...
	template<typename OtherKeyType, typename OtherValueType>
	using ThisContainerFullType				= Map<OtherKeyType, OtherValueType>;
	using ThisFullType						= ThisContainerFullType<KeyType, ValueType>;

	template<bool IsConst>
	using IteratorBase						= container_bases::MapIteratorBase<KeyType, ValueType, IsConst>;
...
public:
	...
	) requires( std::is_copy_constructible_v<ContainedPairType> && std::is_copy_assignable_v<ValueType> )
	{
	...
	constexpr Map(
		const Map																&	other
	) requires( std::is_copy_constructible_v<ContainedPairType> )
	{
		this->Append( other );
	}
	...
	constexpr ValueType																				&	operator[](
		const KeyType																				&	key
		) requires( std::is_copy_constructible_v<ContainedPairType> && std::is_copy_assignable_v<ValueType> && std::is_default_constructible_v<ValueType> )
	{
	...
	constexpr const ContainedPairType																&	Front() const
	{
		BAssert( this->root_node, U"Cannot get container front, container is empty" );
	...
	/// @return
	/// Pair with the smallest key.
	constexpr ContainedPairType																		&	Front()
	{
		BAssert( this->root_node, U"Cannot get container front, container is empty" );
		return this->FindLeftmostFrom( this->root_node )->data;
	}
	...
	constexpr const ContainedPairType																&	Back() const
	{
		BAssert( this->root_node, U"Cannot get container back, container is empty" );
	...
	/// @return
	/// Pair with the largest key.
	constexpr ContainedPairType																		&	Back()
	{
		BAssert( this->root_node, U"Cannot get container back, container is empty" );
		return this->FindRightmostFrom( this->root_node )->data;
	}
	...
	constexpr void																						Append(
		...
		i64																								count					= 1
	) requires( std::is_copy_constructible_v<ContainedPairType> && std::is_same_v<ContainedPairType, typename OtherContainerType::ContainedPairType> )
	{
		...
		BAssert( count > 0, U"Cannot append 0 or negative number of elements" );
	...
	) requires( std::is_move_constructible_v<ContainedPairType> && ( std::is_move_assignable_v<ValueType> || std::is_copy_assignable_v<ValueType> ) )
	{
		auto find_node_result = this->FindNode( pair.first );
		if( find_node_result.found )
		{
	...
	{
		BAssert( this->size, U"Cannot erase from container, container is already empty" );
		BAssert( at.GetContainer() == this, U"Cannot erase from container using iterator that doesn't point to the container we're erasing from" );
		BAssert( at.GetData(), U"Cannot erase from container, iterator was at the end" );
	...
	{
		BAssert( this->size, U"Cannot erase from container, container is already empty" );
		BAssert( from.GetContainer() == this, U"Cannot erase from container using iterator that doesn't point to the container we're erasing from" );
		BAssert( to.GetContainer() == this, U"Cannot erase from container using iterator that doesn't point to the container we're erasing from" );
		BAssert( from.GetData(), U"Cannot erase from container, iterator was at the end" );
...
private:
	...
	{
		BEngineHardAssert( node->right, U"Node has no right child" );
	...
	{
		BEngineHardAssert( !find_node_result.found, U"Node already exists" );
	...
	)
	{
		...
		BEngineHardAssert( !node_parent || ( node == node_parent->left || node == node_parent->right ), U"Cannot remove node from the tree" );
...
// This node has one child which will take over this position.
		...
		else
		{
			...
			BEngineHardAssert( successor, U"Successor not found nor created" );
			BEngineHardAssert( successor->parent, U"Successor has no parent" );
			BEngineHardAssert( successor->left == nullptr, U"Successor has left child which was not expected" );
	...
	constexpr void																						ConstructNode(
		...
		const KeyType																				&	key,
		const ValueType																				&	value
	) requires( std::is_copy_constructible_v<KeyType> && std::is_copy_constructible_v<ValueType> )
	{
	...
	constexpr void																						ConstructNode(
		...
		const KeyType																				&	key,
		ValueType																					&&	value
	) requires( std::is_copy_constructible_v<KeyType> || std::is_move_constructible_v<ValueType> )
	{
	...
	constexpr void																						ConstructNode(
		...
		KeyType																						&&	key,
		const ValueType																				&	value
	) requires( std::is_move_constructible_v<KeyType> && std::is_copy_constructible_v<ValueType> )
	{
	...
	constexpr void																						ConstructNode(
		...
		KeyType																						&&	key,
		ValueType																					&&	value
	) requires( std::is_move_constructible_v<KeyType> && std::is_move_constructible_v<ValueType> )
	{
	...
	{
		BEngineHardAssert( starting_from, U"Starting balancing the tree from nullptr node" );
		...
		while( node )
		{
			...
			BEngineHardAssert( std::abs( this->GetNodeBalance( node ) ) < 2, U"Tree is unbalanced" );
			node = node->parent;
		}
	}
	...
	{
		BEngineHardAssert( old_root, U"Cannot rotate a nullptr node" );
		...
		BEngineHardAssert( new_root, U"Left node was nullptr, cannot rotate" );
		...
		BEngineHardAssert( new_root->parent != old_root->parent, U"Parent was not set correctly" );
		BEngineHardAssert( new_root->right == old_root || new_root->left == old_root, U"Parent does not recognize new child node" );
	...
	) noexcept
	{
		...
		BEngineHardAssert( new_root, U"Right node was nullptr, cannot rotate" );
		...
		BEngineHardAssert( new_root->parent != old_root->parent, U"Parent was not set correctly" );
		BEngineHardAssert( new_root->left == old_root || new_root->right == old_root, U"Parent does not recognize new child node" );
...
public:
	void ScanInvalidPointers( Node * node = nullptr, Node * previous = nullptr ) const
	{
		...
		{
			BEngineHardAssert( 0 && "Node pointer invalid" );
		}
	}
...
static_assert( sizeof( container_bases::MapIteratorBase<u32, u32, true> ) == 16 );
static_assert( sizeof( container_bases::MapIteratorBase<u32, u32, false> ) == 16 );

static_assert( sizeof( Map<u32, u32> ) == 16 );



////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Check if map container fulfills concept requirements.
static_assert( utility::ContainerView<Map<u32, u32>> );
static_assert( utility::ContainerEditableView<Map<u32, u32>> );
static_assert( utility::Container<Map<u32, u32>> );
static_assert( !utility::LinearContainerView<Map<u32, u32>> );
static_assert( !utility::LinearContainerEditableView<Map<u32, u32>> );
static_assert( !utility::LinearContainer<Map<u32, u32>> );
...
{
	{
		...
		map.Erase( 3 );
		map.DebugPrintBinaryTree();
		BEngineHardAssert( map.CheckMaxImbalance() < 2 );
	}
	int test_count = 10000;
	{
		...
		// Iterate backward
		{
			...
			{
				--it;
				BEngineHardAssert( it->first == ( test_count - 1 ) - i );
			}
		}
		BEngineHardAssert( map.CheckMaxImbalance() < 2 );
	}
	...
	}
	{
		...
		// Iterate forward
		{
			...
			while( it != map.end() )
			{
				...
				BEngineHardAssert( current >= previous );
				previous = current;
				++it;
			}
		}
	...
	// Test erase
	{
		...
		}
		{
			for( int i = 0; i < test_count; ++i )
			{
				...
				{
					// Should not exist
					BEngineHardAssert( map.Find( i ) == map.end() );
				}
				else
				{
					// Should exist
	...
	// Test random insert and erase while iterating.
	{
		...
		BEngineHardAssert( baseline.size() == map.Size() );
		BEngineHardAssert( map.CheckMaxImbalance() < 2 );
		{
			auto it = map.begin();
			...
			{
				BEngineHardAssert( it->first == b_it->first );
				++it;
				++b_it;
			}
		}
