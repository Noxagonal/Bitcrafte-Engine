#pragma once

#include <build_configuration/BuildConfigurationComponent.hpp>
#include <core/diagnostic/print_record/PrintRecordSection.hpp>
#include <core/containers/backend/ContainerBase.hpp>
#include <core/memory/MemoryRange.hpp>



namespace bc {
namespace diagnostic {



////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// @brief
/// Represents text that is suitable for logging or other error reporting.
///
/// Provides utilities to construct and manipulate a multi-line and multi-color text entry suitable for printing via the system
/// console, in-editor console, or in-game console.
class BITCRAFTE_ENGINE_API PrintRecord : public container_bases::ContainerResource
{
public:

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	PrintRecord() noexcept = default;
	PrintRecord( const PrintRecord& other ) noexcept;
	PrintRecord( PrintRecord&& other ) noexcept;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	template<i64 CharacterArraySize>
	PrintRecord( const c32( &c_string )[ CharacterArraySize ] ) noexcept
	{
		auto new_section = PrintRecordSection {};
		new_section.SetMessage( c_string, CharacterArraySize ? ( c_string[ CharacterArraySize - 1 ] == '\0' ? CharacterArraySize - 1 : CharacterArraySize ) : 0 );
		AddSection( new_section );
	}

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	~PrintRecord() noexcept;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	auto operator=( const PrintRecord& other ) noexcept -> PrintRecord&;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	auto operator=( PrintRecord&& other ) noexcept -> PrintRecord&;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/// @brief
	/// Adds another PrintRecord at the end of this.
	///
	/// @warning
	/// If print record has been finalized, this function does nothing.
	///
	/// @param other
	/// PrintRecord to append at the end.
	///
	/// @return
	/// Reference to this.
	auto operator+=( const PrintRecord& other ) noexcept -> PrintRecord&;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/// @brief
	/// Adds an individual PrintRecordSection at the end of this.
	/// 
	/// @warning
	/// If print record has been finalized, this function does nothing.
	///
	/// @param section
	/// Section to append at the end.
	///
	/// @return
	/// Reference to this.
	auto operator+=( const PrintRecordSection& section ) noexcept -> PrintRecord&;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/// @brief
	/// Combines this and another PrintRecord and returns the result.
	///
	/// @param other
	/// PrintRecord to add to the end.
	///
	/// @return
	/// A new PrintRecord with combined texts.
	auto operator+( const PrintRecord& other ) const noexcept -> PrintRecord;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	auto GetSectionCount() const noexcept -> i64;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	auto GetSection( i64 index ) const noexcept -> const PrintRecordSection*;
	
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	auto GetSections() const noexcept -> const memory::MemoryRange<const PrintRecordSection>;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/// @brief
	/// Get actual line count of this print record section.
	///
	/// @return
	/// Number of lines taken by this print record.
	auto CalculateLineCount() const noexcept -> i64;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/// @brief
	/// Adds another PrintRecord at the end of this.
	///
	/// @warning
	/// If print record has been finalized, this function does nothing.
	///
	/// @param other
	/// PrintRecord to append at the end.
	///
	/// @return
	/// Reference to this.
	auto Append( const PrintRecord& other ) noexcept -> PrintRecord&;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/// @brief
	/// Adds an individual PrintRecordSection at the end of this.
	/// 
	/// @warning
	/// If print record has been finalized, this function does nothing.
	///
	/// @param section
	/// Section to append at the end.
	///
	/// @return
	/// Reference to this.
	auto AddSection( const PrintRecordSection& section ) noexcept -> PrintRecord&;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/// @brief
	/// Adds an indentation level to this record. Which may be later converted into spaces.
	///
	/// @note
	/// This is fast. Indentation is applied to all PrintRecordSection children by incrementing an integer.
	///
	/// @warning
	/// If print record has been finalized, this function does nothing.
	///
	/// @param add_indentation_level
	/// Value to add to indentation level. Positive values adds more indentation levels, negative values can be used to move back
	/// an indentation.
	///
	/// @return
	/// Reference to this.
	auto AddIndent( u32 add_indentation_level = 1 ) noexcept -> PrintRecord&;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/// @brief
	/// Converts tabs to spaces, calculates number of lines, applies formatting, etc. And returns a copy of this.
	///
	/// This is usually the final stage before being presented to the viewer.
	///
	/// @note
	/// This print record cannot be modified after this step, any changes are ignored.
	///
	/// @note
	/// Print record must be finalized before using it to display information.
	///
	/// @param indentation_size
	/// Number of spaces to be inserted per indentation level. You can think of it as of spaces per tab.
	///
	/// @return
	/// Reference to this.
	auto GetFinalized( u32 indentation_size = 4 ) const noexcept -> PrintRecord;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	auto IsEmpty() const noexcept -> bool;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void Clear() noexcept;

private:

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void Copy( const PrintRecord& other ) noexcept;
	void Swap( PrintRecord& other ) noexcept;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void ResizeNoConstruct( i64 new_count ) noexcept;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/// @brief
	/// Applies the indents to actual spaces in front of every new line and clears recorded indents from PrintRecordSection.
	///
	/// @param processed_section_list
	/// Section list source we should process.
	///
	/// @return
	/// New section list with applied indents.
	void Finalize_ApplyIndents( u32 indentation_size = 4 ) noexcept;

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	PrintRecordSection*		section_list	= {};
	i64						section_count	= {};
};



} // diagnostic
} // bc
